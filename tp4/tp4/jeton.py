class Jeton:
    ...
    def __init__(self, lettre, valeur):
        ...
        if not (len(lettre) == 1 and lettre.isupper() and lettre.isalpha()):
            raise ErreurJetonException('Lettre incorrecte.')
        if not (0 <= valeur <= 20):
            raise ErreurJetonException('Valeur incorrecte.')
    ...
    def __str__(self):
        ...
        if self.valeur < 10:
            ...
            return '{}{}{}'.format(self.lettre, chr(0x2080 + int(self.valeur / 10)),
                                   chr(0x2080 + int(self.valeur % 10)))


class ErreurJetonException(Exception):
    pass
