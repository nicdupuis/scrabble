class Case:
    ...
    def __init__(self, multiplicateur=1, effet=None):
        ...
        if not (1 <= multiplicateur <= 3):
            raise ErreurPlateauException('Multiplicateur incorrect.')
        if not (effet is None or effet in 'ML'):
            raise ErreurPlateauException('Type incorrect.')
...
class ErreurPlateauException(Exception):
    pass
